...
- (void)viewDidLoad
{
    ...
    // title
    self.navigationItem.title = @"炎上NAVERまとめ";
...
{
    if ([[segue identifier] isEqualToString:@"showNaverDetail"]) {
        NSLog(@"showNaverDetail");
        NSIndexPath *indexPath = [self.tableView indexPathForSelectedRow];
        Item *item = _items[indexPath.row];

        NaverDetailViewController *naverDetailViewController = [segue destinationViewController];
        [[segue destinationViewController] setDetailItem:item];
        naverDetailViewController.linkurl = item.linkurl;
        naverDetailViewController.imgurl = item.imgurl;
...
- (void)startDownload{
    ...
    //　NSString * feed = @"http://matome.naver.jp/feed/hot";
    NSString * feed = @"http://matome.naver.jp/feed/topic/1Himf";
...
- (void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary *)attributeDict{
    ...
    if([_elementName isEqualToString:@"media:thumbnail"]){
        //NSLog(@"url:%@", [attributeDict objectForKey:@"url"]);
        _item.imgurl = [attributeDict objectForKey:@"url"];
    }
}
...
- (void)parser:(NSXMLParser *)parser foundCharacters:(NSString *)string{
    if([_elementName isEqualToString:@"title"]){
        ...
        //_item.imgurl = [_item.imgurl stringByAppendingString:string];
